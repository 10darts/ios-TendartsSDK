...
+(void) startUpdating
{
	...
	//[me.locationManager startMonitoringSignificantLocationChanges];
}
+(void) stopUpdating
{
	...
	//[me.locationManager stopMonitoringSignificantLocationChanges];
...
/*
 - (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
 {
 NSNumber *lat = [NSNumber numberWithDouble:[[locations lastObject] coordinate].latitude];
 NSNumber *lon = [NSNumber numberWithDouble:[[locations lastObject] coordinate].longitude];
 NSDictionary *coordinate = @{@"lat":lat, @"lon":lon};
 
 NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
 [userDefault setObject:coordinate forKey:@"coordinate"];
 [userDefault synchronize];
 ...
 [reverseGeocoder reverseGeocodeLocation:[locations lastObject]
 completionHandler:^(NSArray *placemarks, NSError *error){
 [userDefault setObject:[[placemarks firstObject] ISOcountryCode]
 forKey:@"currentCountryCode"];
 [userDefault setObject:[[placemarks firstObject] locality]
 forKey:@"currentCity"];
 [userDefault synchronize];
 }];
